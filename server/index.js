...
app.post('/order_material', (req, res) => {
  ...
  db.query(
    ...
    request.post({
      ...
      form: {
          message: username+' มีคำร้องขอเบิกวัสดุใหม่ ชื่อวัสดุ:'+material_name+' รหัสวัสดุ:'+material_Id
      }
...
app.post('/order_durablearticles', (req, res) => {
  ...
  db.query(
    ...
    request.post({
      ...
      form: {
          message: username+' มีคำร้องขอเบิกครุภัณฑ์ใหม่ ชื่อครุภัณฑ์'+durablearticles_name+'รหัสครุภัณฑ์:'+durablearticles_Id
      }
    }, (err, httpResponse, body) => {
      if (err) {
          console.error(err);
...
app.post('/upload', upload.single('photo'), function(req, res, next) {
  ...
  const { durablearticles_Id,du_name, room, repair_detail, Informer } = JSON.parse(req.body.data);
  ...
  request.post({
    ...
    form: {
        message: Informer +" บันทึกการซ่อมใหม่!"+" ชื่อครุภัณ์:"+du_name+" id:"+durablearticles_Id
    }
  }, (err, httpResponse, body) => {
    if (err) {
        console.error(err);
